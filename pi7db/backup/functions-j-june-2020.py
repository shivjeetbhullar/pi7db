import json,os,datetime,glob,random,orjson,pickle
 ...
 if len(dic_data):
  ...
  for up_path in all_update:
    if not up_path[-9:] == "pi7dbauto":
      ...
      for x in commands:
         if isinstance(commands[x],dict) and increment_v in commands[x]:js_data[x] = js_data[x]+commands[x][increment_v]
         elif isinstance(commands[x],dict) and decrement_v in commands[x]:js_data[x] = js_data[x]-commands[x][decrement_v]
      ...
      if isinstance(v_data,list):
        for x_data in v_data:
          for x in commands:
            if isinstance(commands[x],dict) and increment_v in commands[x]:x_data[x] = x_data[x]+commands[x][increment_v]
            elif isinstance(commands[x],dict) and decrement_v in commands[x]:x_data[x] = x_data[x]-commands[x][decrement_v]
